...
import { getProductGrowthQuality } from './growth-quality';
...
export async function getProducts(limit: number = 50, _filters?: ProductFilters): Promise<Product[]> {
  void _filters;
  const growthQuality = await getProductGrowthQuality(limit);

  return growthQuality.map(product => ({
    quickBooksInternalId: product.sku,
    itemName: product.sku,
    salesDescription: '',
    productFamily: product.productFamily,
    materialType: product.materialType,
    salesPrice: '0.00',
    purchaseCost: product.inventoryValueAtCost || '0.00',
    marginPercentage: product.grossMarginPercentage || '0.0',
    marginAmount: product.grossMarginAmount || '0.00',
    actualMarginPercentage: product.grossMarginPercentage || undefined,
    actualMarginAmount: product.grossMarginAmount || undefined,
    grossMarginAmount: product.grossMarginAmount,
    discountLeakageAmount: product.discountLeakageAmount,
