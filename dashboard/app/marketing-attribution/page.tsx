    ...
    <Card className="rounded-md py-0 shadow-none">
      <CardHeader className="border-b px-3 py-2">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Revenue concentration and order quality by acquisition source</p>
          </div>
      ...
      <CardContent className="p-0">
        {leaders.map((channel) => {
          ...
          return (
            <div key={channel.acquisitionChannel} className="grid grid-cols-[minmax(0,1fr)_9rem] items-center gap-3 border-b px-3 py-2 last:border-b-0">
              <div className="min-w-0">
      ...
      <CardHeader className="border-b px-3 py-2">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Highest UTM-tracked demand in the selected period</p>
          </div>
      ...
      <CardContent className="p-0">
        {leaders.length > 0 ? (
          leaders.map((campaign) => {
            ...
            return (
              <div key={`${campaign.utmSource || 'none'}-${campaign.utmMedium || 'none'}-${campaign.utmCampaign || 'none'}`} className="border-b px-3 py-2 last:border-b-0">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[11px] text-muted-foreground">{formatNumber(campaign.orderCount, 0)} orders</p>
                  </div>
                </div>
                <div className="mt-2 flex items-center gap-2">
                  <InlineBar value={(revenue / maxRevenue) * 100} tone="green" />
                  <span className="w-12 text-right font-mono text-[11px] text-muted-foreground">{campaign.optInRate}% opt</span>
                </div>
              </div>
            );
          })
        ) : (
          <div className="px-3 py-8 text-center text-sm text-muted-foreground">No UTM campaign data for this period</div>
        )}
      </CardContent>
    </Card>
  );
            ...
            return (
              <div key={`${title}-${name}`} className="min-w-0">
                ...
                <div className="mt-1 flex items-center gap-2">
                  ...
                  <span className="w-16 text-right text-[11px] text-muted-foreground">{formatNumber(row.orderCount, 0)} orders</span>
                </div>
              </div>
            );
          })
      ...
      <CardHeader className="border-b px-3 py-2">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Where attributed demand enters and originates</p>
          </div>
      ...
      <CardHeader className="border-b px-3 py-2">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Latest completed month compared with prior available month</p>
          </div>
      ...
      <CardContent className="p-0">
        {latest ? (
          leadingChannels.map((channel) => {
            ...
            return (
              <div key={channel.acquisitionChannel} className="grid grid-cols-[minmax(0,1fr)_8rem] items-center gap-3 border-b px-3 py-2 last:border-b-0">
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium">{channel.acquisitionChannel}</p>
                  <p className="text-[11px] text-muted-foreground">{formatCompactCurrency(currentRevenue)} latest month</p>
                </div>
                <div className={cn('text-right font-mono text-xs font-semibold', change == null ? 'text-muted-foreground' : change >= 0 ? 'text-emerald-700' : 'text-red-700')}>
          ...
          <div className="px-3 py-8 text-center text-sm text-muted-foreground">No monthly trend data for this period</div>
        )}
      </CardContent>
    </Card>
  );
            ...
            <CardHeader className="border-b px-3 py-2">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">UTM-tracked campaign results for {periodLabel.toLowerCase()}</p>
                </div>
        ...
        <section className="grid gap-3 md:grid-cols-3">
          <Card className="rounded-md py-0 shadow-none">
            <CardContent className="flex items-center gap-3 p-3">
              ...
              <div className="min-w-0">
                <p className="text-xs text-muted-foreground">Average per Channel</p>
          ...
          <Card className="rounded-md py-0 shadow-none">
            <CardContent className="flex items-center gap-3 p-3">
              <div className="grid h-8 w-8 place-items-center rounded-md bg-background">
              ...
              <div className="grid min-w-0 grid-cols-3 gap-3 text-xs">
                ...
                <div>
                  <p className="text-muted-foreground">Referrers</p>
