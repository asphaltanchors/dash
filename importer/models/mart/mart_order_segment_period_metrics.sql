/*
ABOUTME: Four trailing-year customer segment metrics for the orders dashboard.
ABOUTME: Provides one row per customer segment and rolling annual period for trend and share display.
*/

{{ config(
    materialized = 'table',
    tags = ['dashboard', 'orders', 'segments', 'current']
) }}
...
period_orders AS (
    ...
    WHERE o.total_amount IS NOT NULL
        ...
        AND o.customer_segment <> ''
        AND o.customer_segment <> 'Export'
    GROUP BY
        p.period_index,
        p.period_start,
        p.period_end,
